...
All-in-One 包含标准 XSwitch 所有的功能，默认许可证可以支持 5 个并发 Channel，如果需要更多并发请联系我们购买正式版。

本版本以 Docker 镜像方式提供，可以安装到 Linux、Windows、macOS 操作系统上。

# 声明

XSwitch 版权归烟台小樱桃网络科技有限公司所有。本发行版提供 Docker 镜象，任何人都可以使用，没有时间限制。本版本仅供学习和自用，不允许二次分发和售卖（但可以告诉你的客户自己下载，你可以教他使用）。使用时须保证本镜像的完整性，不准使用任何方式对本系统进行逆向、破解等，不准以任何方式向别人提供镜像中的部分文件或组件。
